...
- The settings UI now covers speech mode (`classic` / `live` / `realtime` experimental), the optional realtime fallback toggle, audio format, first-chunk lead-in, speech playback speed, translation target language, STT provider selection (`webview2` / `openai_online` / `openai_whisper_local`), optional compare-all debug mode, STT language hint, and the OpenAI API key.
...
- STT-Vergleichsschicht mit drei Varianten: `webview2`, `OpenAI Speech-to-Text API` (`gpt-4o-transcribe` über Realtime-Transcription) und `lokales Whisper (offline)`
- STT-Debug-Logs mit allen ermittelten Transkripten, Latenzen und Fehlerdetails pro Provider
